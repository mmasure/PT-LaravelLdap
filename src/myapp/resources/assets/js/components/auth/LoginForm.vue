<template>
    <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
        ...
        <div class="form-group has-feedback" :class="{ 'has-error': form.errors.has('email') }" v-if="type === 'email'">
            <input type="email" class="form-control" :placeholder="placeholder" :name="name" value=""
                   v-model="form.email" @change="adddomain" autofocus/>
        ...
        <div class="form-group has-feedback" :class="{ 'has-error': form.errors.has('username') }" v-else>
            <input type="text" class="form-control" :placeholder="placeholder" :name="name" v-model="form.username"
                   @change="adddomain" autofocus/>
            <span class="glyphicon form-control-feedback" :class="[icon]"></span>
            <transition name="fade">
        ...
        <div class="row">
            ...
            <div class="col-xs-4">
                <button type="submit" class="btn btn-primary btn-block btn-flat btn-submit"
                        v-text="trans('adminlte_lang_message.buttonsign')" :disabled="form.errors.any()">
                </button>
            </div>
        </div>
...
<!--        <div class="row">-->
<!--            <div class="col-xs-12">-->
<!--                <ToggleButton id="label_example" v-on:change="triggerEvent"/>-->
<!--                LDAP-->
<!--            </div>-->
<!--        </div>-->
    </form>
...
export default {
    ...
    methods: {
        submit() {
            $('.btn-submit').html('<i class="fa fa-refresh fa-spin"></i>')
